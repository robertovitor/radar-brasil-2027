...
RESERVAS = ROOT / "instagram/reservas-publicacao.json"
...
def git_sync() -> bool:
    proc = run(["git", "pull", "--rebase", "--autostash", "origin", "main"])
...
def batch_key(batch_path: pathlib.Path) -> str:
    ...
    if not posts:
        return ""
    try:
        post = json.loads((ROOT / posts[0]).read_text(encoding="utf-8"))
    except Exception:
        return ""
...
def main() -> int:
    ...
    for attempt in range(1, MAX_TENTATIVAS + 1):
        ...
        if not git_commit_push(["instagram/artes", "instagram/fila/automatica"], "Prepara post com imagem única"):
            print("continuation_error=prepare_push_failed", file=sys.stderr)
            return 1

        if not git_sync():
            return 1
        reserve = run([sys.executable, "scripts/reservar_publicacao_instagram.py", "reserve", "--batch", str(batch.relative_to(ROOT))])
        if reserve.returncode != 0:
            print("continuation_error=reservation_failed", file=sys.stderr)
            return 1
        ...
        publish = run([
            sys.executable,
            "scripts/publicar_lote_instagram.py",
        ...
        if not git_commit_push(["instagram/publicados.json"], "Registra publicação confirmada no Instagram"):
            return 1
        if RATE_STATE.exists() and run(["git", "status", "--porcelain", "--", "instagram/meta-rate-limit.json"], capture=True).stdout.strip():
        ...
        if confirmed:
            if not git_sync():
                return 1
            clear = run([sys.executable, "scripts/reservar_publicacao_instagram.py", "clear", "--batch", str(batch.relative_to(ROOT))])
