...
def fit_title_complete(draw, title, width, start_size=88, min_size=58, max_lines=4):
    ...
    for pattern in (
        r'\s+por\s+exig[eê]ncia\s+.*    # mantém o comportamento fail-closed do gate em vez de aprovar arte ilegível.
...
,
        ...
        if len(reduced) >= 28 and reduced not in clause_candidates:
            clause_candidates.append(reduced)

    for candidate in clause_candidates:
        for size in range(66, min_size - 1, -2):
            f = base.font(size, True)
            lines = base.wrap(draw, candidate, f, width)
            if len(lines) <= max_lines:
                TITLE_RENDER_META[original] = {
                ...
                if candidate != original:
                    print('title_shortened_automatically=true')
                    print('title_fit_fallback=closed_headline')
                    print('art_title=' + candidate)
                return f, lines, True
